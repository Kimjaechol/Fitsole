 ...
 *   - resolution_report table (method / confidence / note)
 *   - warnings alert list
 *
...
function buildDimensionRows(
  dims: DimensionMap | undefined,
): Array<{ key: string; label: string; value: DimensionValue }> {
  if (!dims) return [];
...
export default function ShoeMergeClient() {
  ...
  const resolutionReportEntries = useMemo(() => {
  ...
  function MergeResultCard({ result }: { result: MergeResponse }) {
  ...
  function GradingSection({ result }: { result: MergeResponse }) {
  ...
  return (
    <div className="space-y-6">
      <UploadForm />
      {mergeResult && <MergeResultCard result={mergeResult} />}
      {mergeResult && <GradingSection result={mergeResult} />}
    </div>
  );
}
